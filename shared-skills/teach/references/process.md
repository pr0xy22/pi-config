...
- **Claude Code only** (the harness has the `AskUserQuestion` tool — see the detection table in [quiz-ui.md](quiz-ui.md)): batch the `.alvar/maps` and `.alvar/sessions` file writes at natural stopping points — end of session, or a long pause — instead of after every single quiz. Track taught/quiz/result in reasoning as it happens so nothing is lost; just delay the write. This works around latency in one specific learner's external log/viewer setup and does not apply to other harnesses (Grok, Codex, OpenCode, Pi), which should keep writing these files as they go, per the default rule above.
