...
- Math notation: always write math in LaTeX (`$\frac{1}{2}$`, `$-\frac{1}{2}$`, etc.), never plain text like `1/2`. Applies everywhere — prose, worked examples, and quiz question stems/options alike.
- Currency is NOT math: write dollar amounts as plain text with an escaped dollar (`\$250`), never wrapped in math delimiters (`$\$250$`). Math-wrapped currency misparses in markdown renderers and drags surrounding prose into math mode.
