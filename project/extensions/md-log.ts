...
export default function mdLog(pi: ExtensionAPI) {
	...
	function assistantBlock(text: string): string {
		return callout("abstract", "PI", text.split("\n"));
	}
	...
	function questionCallout(label: string, question: string, context: string | undefined, options: Array<{ label: string }>): string {
		...
		const type = options.length > 0 ? "quiz" : "question";
		const title = label === "Quiz" ? "Quick check" : label;
	...
	function answerCalloutQuiz(details: any): string {
		...
		const selections = dontKnow
			? ["- **Answer.** I don't know"]
			: answers.length > 0
				? answers.map((a) => `- **${a.index}.** ${a.label}`)
				: ["- **Answer.** (none)"];
		const selectionBlock = callout(
			"answer",
			answers.length === 1 || dontKnow ? "Your selection" : "Your selections",
			selections,
		);
		const body: string[] = [];
		...
		body.push(`Correct answer: **${correctStr || "(not provided)"}**`);

		// Machine-readable payload for viewers. The prose above is for humans;
		// this HTML comment is the stable contract — parse this, not the prose.
		const selectedIndices: number[] = dontKnow
			? []
			: answers.map((a) => a?.index).filter((i: any) => typeof i === "number");
		body.push(`<!-- qa-result ${JSON.stringify({ selected: selectedIndices, correct: correctIndices, dontKnow })} -->`);
	...
	function answerCalloutAsk(details: any): string {
		...
		const body: string[] = answers.map((a) => {
			...
			if (a.type === "text") return `- **Answer.** ${a.label}`;
			return `- **${a.index}.** ${a.label}`;
		});
		if (body.length === 0) body.push("(no answer)");
		// Machine-readable payload — same contract as answerCalloutQuiz.
		const selectedIndices: number[] = answers
			.map((a) => a?.index)
			.filter((i: any) => typeof i === "number");
		body.push(`<!-- qa-result ${JSON.stringify({ selected: selectedIndices })} -->`);
		return callout("answer", answers.length === 1 ? "Your selection" : "Your selections", body);
	}
